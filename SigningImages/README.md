...
Примечаниед генерацией ключей установите один из pinentry агентов. Например:
```
...
### Запуск регистратора как podman-контейнера
...
роверьте командой curl работу регистратора:
```
# curl -i http://registry.local
HTTP/1.1 200 OK
Cache-Control: no-cache
...
Второй элемент `sigstore` использет демон crio kubernetes версии < 1.26.
...
создает пользователя (например user) входящего в группу podman:sudo adduser user
sudo passwd user
