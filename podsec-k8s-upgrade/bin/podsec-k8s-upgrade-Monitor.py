...
class Nodes:
  ...
  def __init__(self):
    ...
    # print('data=', data)
    self.data = {}
    for nodeInfo in data['items']:
      nodeName = nodeInfo['metadata']['name']
      self.data[nodeName] = nodeInfo
    fp.close()
    self.controlPlanes = []
    self.workers = []
    for nodeName in self.data:
      # key = self.data[nodeName]['spec']['taints'][0]['key']
      # print(key)
      if 'taints' in self.data[nodeName]['spec'] and \
        self.data[nodeName]['spec']['taints'][0]['key'] == 'node-role.kubernetes.io/control-plane':
        self.controlPlanes.append(nodeName)
      else:
        self.workers.append(nodeName)
...
class NodesMonitor :
  ...
  def untaint(self):
    for masterName in self.initState.controlPlanes:
      cmd = 'kubectl taint nodes %s node-role.kubernetes.io/control-plane:NoSchedule-' %

# MAIN
nodesMonitor = NodesMonitor()
# print(json.dumps(nodesMonitor.initState.data, indent=2))
print('controlPlanes=', json.dumps(nodesMonitor.initState.controlPlanes, indent=2))
print('workers=', json.dumps(nodesMonitor.initState.workers, indent=2))
