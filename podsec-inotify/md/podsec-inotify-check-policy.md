...
### Запуск сервиса через cron

Кроме запуска скирпта через `nagios` скрипт может запускаться через `crond`.
В состав пакета входит файл для `cron` `/etc/podsec/crontabs/podsec-inotify-check-policy`. Файл содержит единственную строку с описанием режима запуска скрипта `podsec-inotify-check-policy`.
